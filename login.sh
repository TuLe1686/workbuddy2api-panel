...
if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER}$"; then
    ...
    sleep 2
    # /status 属管理面：优先读 panel_key，未设置时回落 api_key（兼容旧配置）
    ADMIN_KEY=$(python3 -c "import json; d=json.load(open('config.json')); print(d.get('panel_key') or d.get('api_key',''))" 2>/dev/null)
    COUNT=$(curl -s http://127.0.0.1:7863/status -H "Authorization: Bearer ${ADMIN_KEY:-test_key}" 2>/dev/null | python3 -c "import json,sys; print(len(json.load(sys.stdin).get('accounts',[])))" 2>/dev/null || echo "?")
